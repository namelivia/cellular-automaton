...
class World(object):
    ...
    def __init__(self, surface, width, tile, outer):
        ...
        for idx in range(0, width):
            self.cells.append(
                Cell(self.surface, self.INITIAL_STATUS, idx, 0, self.tile)
            )
    ...
    def step(self, index):
        ...
        for idx, val in enumerate(self.cells):
            ...
            right_status = (
                self.cells[idx + 1].status if idx < len(self.cells) - 1 else self.outer
            )
            new_cells.append(
                Cell(
                    self.surface,
                    val.step(left_status, right_status),
                    idx,
                    index,
                    self.tile,
                )
...
class Cell(object):
    ...
    def step(self, left_status, right_status):
        ...
        return self.get_new_status(
            str(left_status) + str(self.status) + str(right_status)
        )
    ...
    def draw(self):
        ...
        sdl2.ext.fill(
            self.surface, color, (self.x_coord, self.y_coord, self.tile, self.tile)
        )
